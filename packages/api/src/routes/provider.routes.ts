...
router.get('/providers/:id/public-profile', 
...
router.post('/providers',
  [
    ...
    body('primaryLicenseType').notEmpty().withMessage('Primary license type is required'),
    ...
    body('acceptsReferrals').optional().isBoolean().withMessage('Accepts referrals must be a boolean'),
    body('responseTimeHours').optional().isInt({ min: 1, max: 168 }).withMessage('Response time must be between 1 and 168 hours'),
  ],
  validate([]),
...
router.get('/providers/:id',
  param('id').isUUID().withMessage('Invalid provider ID'),
  query('includeHomes').optional().isBoolean().withMessage('includeHomes must be a boolean'),
  query('includeServices').optional().isBoolean().withMessage('includeServices must be a boolean'),
  query('includeOpenings').optional().isBoolean().withMessage('includeOpenings must be a boolean'),
  validate([]),
...
router.put('/providers/:id',
  ...
  [
    ...
    body('verificationNotes').optional().isString().isLength({ max: 1000 }).withMessage('Verification notes must be less than 1000 characters'),
  ],
  validate([]),
...
router.put('/providers/:id/profile',
  param('id').isUUID().withMessage('Invalid provider ID'),
  [
    body('description').optional().isString().isLength({ max: 2000 }).withMessage('Description must be less than 2000 characters'),
    body('logo').optional().isURL().withMessage('Logo must be a valid URL'),
...
// License management
router.post('/providers/:providerId/licenses',
  ...
  [
    ...
    body('issuingState').optional().isLength({ min: 2, max: 2 }).withMessage('Issuing state must be 2 characters'),
    body('issueDate').isISO8601().withMessage('Valid issue date is required'),
    body('expirationDate').isISO8601().withMessage('Valid expiration date is required'),
    body('documentUrl').isURL().withMessage('Document URL must be a valid URL'),
  ],
  validate([]),
  providerController.uploadLicense
);

router.put('/licenses/:licenseId/verify',
  param('licenseId').isUUID().withMessage('Invalid license ID'),
  [
    body('status').isIn(['PENDING', 'ACTIVE', 'EXPIRED', 'SUSPENDED', 'REJECTED']).withMessage('Invalid license status'),
    body('verificationNotes').optional().isString().isLength({ max: 1000 }).withMessage('Verification notes must be less than 1000 characters'),
  ],
  validate([]),
...
router.get('/providers/:providerId/licenses',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
  query('status').optional().isIn(['PENDING', 'ACTIVE', 'EXPIRED', 'SUSPENDED', 'REVOKED']).withMessage('Invalid license status'),
  validate([]),
  providerController.getProviderLicenses
);

// Update license
router.put('/providers/:providerId/licenses/:licenseId',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
  param('licenseId').isUUID().withMessage('Invalid license ID'),
  [
    body('licenseType').optional().notEmpty().withMessage('License type cannot be empty'),
...
// Delete license
router.delete('/providers/:providerId/licenses/:licenseId',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
  param('licenseId').isUUID().withMessage('Invalid license ID'),
  validate([]),
...
router.get('/providers/by-user/:userId',
  param('userId').isUUID().withMessage('Invalid user ID'),
  validate([]),
  providerController.getProviderByUserId
);

router.get('/providers/organization/:organizationId',
...
router.put('/providers/:providerId/services',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
  [
    body('serviceIds').isArray().withMessage('serviceIds must be an array'),
    body('serviceIds.*').isUUID().withMessage('Each service ID must be a valid UUID'),
  ],
  validate([]),
...
router.get('/providers/:providerId/referrals',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
  [
...
router.get('/providers/:providerId/staff',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
  validate([]),
  providerController.getOrganizationStaff
);

router.post('/providers/:providerId/staff',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
  [
...
router.delete('/providers/:providerId/staff/:staffUserId',
  param('providerId').isUUID().withMessage('Invalid provider ID'),
