...
export default function ProviderSettingsPage() {
  ...
  const refreshSubscriptionData = async () => {
    ...
    try {
      const [providerData, subscriptionData] = await Promise.all([
        providerService.getProviderByOrganizationId(user.organizationId),
      ...
      try {
        ...
        if (providerData?.id) {
          ...
          form.reset({
            ...
            acceptsReferrals: providerData.acceptsReferrals ?? true,
            responseTimeHours: providerData.responseTimeHours || undefined,
          });

          // Set logo and cover image if they exist
          if (providerData.logo) {
            // Determine mimeType from URL if possible
            const logoUrl = providerData.logo.toLowerCase();
          ...
          if (providerData.coverImage) {
            // Determine mimeType from URL if possible
            const coverUrl = providerData.coverImage.toLowerCase();
  ...
  const handleSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      const updatedProvider = await providerService.getProviderByOrganizationId(
        user!.organizationId!
      );
