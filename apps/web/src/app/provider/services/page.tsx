...
export default function ProviderServicesPage() {
  ...
  const { tier, limits, canAddServices, getRemainingServices } =
    useSubscription();
  ...
  // Fetch data when provider ID is available
  useEffect(() => {
    if (providerId) {
  ...
  const fetchProviderServices = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setSelectedServices(
          response.data.map((ps: ProviderServiceType) => ps.serviceId)
        );
      } else {
        setError(response.message || "Failed to load provider services");
      }
    } catch (err) {
      console.error("Error fetching provider services:", err);
      setError(
        err instanceof Error ? err.message : "Failed to fetch provider services"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchAvailableServices = async () => {
    ...
    try {
      // Pass providerId to filter services based on provider's licenses
      const response = await providerService.getAvailableServices(providerId);
      if (response.success && response.data) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await fetchProviderServices(); // Refresh provider services
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <Card variant="healthcare">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant={
                  canAddServices(selectedServices.length)
                    ? "healthcareSuccess"
                    : "healthcareWarning"
                }
              >
        ...
        <CardContent className="space-y-4">
          {!canAddServices(selectedServices.length) && tier === "FREE" && (
      ...
      {tier === "FREE" &&
        selectedServices.length >= limits.maxServices * 0.8 && (
          <UpgradeBanner
