...
export default function HospitalSWOnboardingPage() {
  ...
  useEffect(() => {
    const loadExistingData = async () => {
      ...
      if (!user) {
        // User is still loading, don't set isLoading to false yet
        return;
      }
      ...
      try {
        ...
        // Load hospital staff first to get organizationId
        const hospitalStaffResponse =
          await hospitalStaffService.getHospitalStaffByUserId(user.id);
        const hospitalStaffData = hospitalStaffResponse || null;

        // Store hospital staff data for later use
        setHospitalStaffData(hospitalStaffData);

        // Get organizationId from hospital staff if user doesn't have it
        const orgId = user.organizationId || hospitalStaffData?.organizationId;
        ...
        setOnboardingState({
          ...
          organizationData: organizationData
            ? {
                ...
                addressLine2: organizationData.addressLine2 || undefined,
                ...
                ein: organizationData.ein || undefined,
                fax: organizationData.fax || undefined,
              }
            : undefined,
        });
      } catch (error) {
    ...
    async (
      ...
      try {
        ...
        setOnboardingState((prev) => ({
          ...prev,
          organizationData: step === 0 ? data : prev.organizationData,
          completedSteps:
        ...
        // Get organizationId - try from user, or fetch from hospital staff
          ...
          if (orgId) {
            ...
            try {
              await organizationService.updateOrganization(orgId, {
                ...
                addressLine2: orgData.addressLine2 || undefined,
                city: orgData.city,
                state: orgData.state,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (onboardingState.organizationData && orgId) {
        await organizationService.updateOrganization(orgId, {
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-foreground">
                Hospital Social Worker Onboarding
              </h1>
              <p className="text-muted-foreground">
                Complete your profile to start managing discharge cases
              </p>
            </div>
          ...
          <Card variant="healthcare">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {currentStep === 1 && <Check className="w-5 h-5" />}
