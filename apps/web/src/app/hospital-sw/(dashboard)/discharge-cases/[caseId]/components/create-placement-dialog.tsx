...
export function CreatePlacementDialog({
  ...
  // Filter invitations to only show providers who have ACCEPTED
  const acceptedProviders = invitations.filter(
    (inv) => inv.response === "ACCEPTED"
  );
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await placementService.createPlacementFromDischargeCase({
        dischargeCaseId,
  ...
  const selectedProvider = acceptedProviders.find(
    (inv) => inv.providerId === selectedProviderId
  );
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              No providers have accepted the invitation yet.
            </DialogDescription>
          </DialogHeader>
          <div className="py-6 text-center text-muted-foreground">
            <p>
              Wait for providers to accept the invitation before creating a
              placement.
            </p>
          </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Create Placement from Discharge Case</DialogTitle>
          <DialogDescription>
            Step {step} of 4: {
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    {acceptedProviders.map((invitation) => (
                      <SelectItem
                        key={invitation.providerId}
                        value={invitation.providerId}
                      >
                        {invitation.provider?.organization?.name || "Unknown Provider"}
                        {invitation.respondedAt && (
                          <span className="text-xs text-muted-foreground ml-2">
                            (Accepted {format(new Date(invitation.respondedAt), "MMM d")})
                          </span>
                        )}
                      </SelectItem>
                    ))}
                ...
                <p className="text-sm text-muted-foreground">
                  Only showing providers who have accepted the invitation
                </p>
