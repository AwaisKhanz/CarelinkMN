  ...
  const handleSubmit = async (data: ClientFormData) => {
    ...
    try {
      ...
      const clientData = {
        ...
        servicesNeeded: Array.isArray(data.servicesNeeded) ? data.servicesNeeded : (typeof data.servicesNeeded === "string" ? data.servicesNeeded.split("\n").map((s) => s.trim()).filter(Boolean) : []),
        workHistory: workHistoryJson,
        skills: Array.isArray(data.skills) ? data.skills : (typeof data.skills === "string" ? data.skills.split("\n").map((s) => s.trim()).filter(Boolean) : []),
        interests: Array.isArray(data.interests) ? data.interests : (typeof data.interests === "string" ? data.interests.split("\n").map((s) => s.trim()).filter(Boolean) : []),
        status: data.status,
        assignedSpecialistId: data.assignedSpecialistId || undefined,
      };

      const response = await vrsService.createClient(clientData);
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <Card variant="healthcare">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="servicesNeeded">Services Needed (one per line)</Label>
              <Textarea
                id="servicesNeeded"
                rows={3}
