...
const jobSchema = z.object({
  ...
  schedule: z.union([z.array(z.string()), z.string()]).default([]),
  wage: z.string().min(1, "Wage is required"),
  wageType: z.string().min(1, "Wage type is required"),
  requirements: z.union([z.array(z.string()), z.string()]).default([]),
  preferredSkills: z.union([z.array(z.string()), z.string()]).default([]),
...
function EditJobPageContent() {
    ...
    const fetchJob = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          // Format dates
          const expiresAt = jobData.expiresAt
          ...
          form.reset({
            ...
            wage: String(jobData.wage),
            wageType: jobData.wageType,
  ...
  }, [jobId, form, setTitle, setDescription]);
    ...
    try {
      const jobData = {
        ...
        preferredSkills: Array.isArray(data.preferredSkills)
          ...
          : typeof data.preferredSkills === "string"
            ? data.preferredSkills
                .split("\n")
                .map((s) => s.trim())
                .filter(Boolean)
      ...
      const response = await vrsService.updateJob(jobId, jobData);
  ...
  if (isLoading) {
    return <VRSLoadingState message="Loading job..." />;
  }
