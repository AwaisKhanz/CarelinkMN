...
interface BatchAddToShortlistDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  referralIds: string[];
  referralNumbers: string[];
  onConfirm: (providerIds: string[], notes?: string) => Promise<void>;
  isAdding: boolean;
}

export function BatchAddToShortlistDialog({
  open,
  onOpenChange,
  referralIds,
  referralNumbers,
  onConfirm,
  isAdding,
}: BatchAddToShortlistDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    await onConfirm(selectedProviders, notes || undefined);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Batch Add to Shortlist</DialogTitle>
          <DialogDescription>
            Select providers to add to {referralIds.length} referral
            {referralIds.length !== 1 ? "s" : ""}: {referralNumbers.join(", ")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 flex flex-col gap-4 min-h-0">
          ...
          {/* Notes */}
          <div>
            <Label htmlFor="batch-shortlist-notes">
              Notes (optional) - Will be added to all shortlist entries
            </Label>
            <Textarea
              id="batch-shortlist-notes"
              placeholder="Add notes for these shortlist entries..."
              value={notes}
          ...
          <div className="flex justify-end gap-2 pt-2 border-t">
            ...
            <Button
              ...
              disabled={isAdding || selectedProviders.length === 0}
            >
              {isAdding ? (
                <>
                ...
                <>
                  ...
                  Add to Shortlist ({selectedProviders.length} provider
