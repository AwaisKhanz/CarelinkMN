...
function CaseManagerPlacementDetailPageContent() {
      ...
      <div className="container mx-auto p-6">
        <Card>
          <CardContent className="py-12 text-center">
            <p className="text-destructive">{error || "Placement not found"}</p>
            <Button
              variant="outline"
              onClick={() => router.back()}
              className="mt-4"
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Manage placement details and coordination
            </p>
          </div>
        </div>
      ...
      {/* Patient Information (from Referral or Discharge Case) */}
      {(placement.referral || placement.dischargeCase) && (
        <Card variant="healthcare">
          <CardHeader>
            <CardTitle>Client Information</CardTitle>
            <CardDescription>
              Information from {placement.referral ? "referral" : "discharge case"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">
                  {placement.referral?.clientInitials || placement.dischargeCase?.patientInitials}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground">Age</p>
                <p className="font-medium">
                  {placement.referral?.clientAge || placement.dischargeCase?.patientAge} years
                </p>
              </div>
              <div>
                <p className="text-muted-foreground">Gender</p>
                <p className="font-medium">
                  {placement.referral?.clientGender || placement.dischargeCase?.patientGender}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground">Reference ID</p>
                <p className="font-medium">
                  {placement.referral?.referralNumber || placement.dischargeCase?.caseNumber}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="family" className="mt-6">
          <FamilyTab placementId={placement.id} readOnly={true} />
        </TabsContent>

        <TabsContent value="updates" className="mt-6">
          <UpdatesTab placementId={placement.id} readOnly={true} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
export default function CaseManagerPlacementDetailPage() {
  return (
    <RequirePermission
    ...
    >
      <CaseManagerPlacementDetailPageContent />
    </RequirePermission>
  );
}
