...
export function usePageMetadata() {
  ...
  if (!context) {
    throw new Error("usePageMetadata must be used within PublicDashboardLayout");
  }
  return context;
}

export function PageMetadataProvider({ children }: { children: ReactNode }) {
  const [title, setTitle] = useState("Find Care");
  const [description, setDescription] = useState(
    "Search for care providers in Minnesota"
  );
