...
interface ProviderSubscriptionGuardProps {
  children: ReactNode;
...
/**
 * Provider-specific subscription guard
 * Checks subscription tier before allowing access to features
 * Provides upgrade prompts when access is denied
 */
export function ProviderSubscriptionGuard({
  children,
  ...
  redirectPath,
  fallback,
}: ProviderSubscriptionGuardProps) {
  ...
  useEffect(() => {
    ...
    if (!hasAccess && redirectOnFail) {
      const path = redirectPath || "/provider/settings?tab=subscription";
      router.push(path);
      return;
    }
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <Badge variant="healthcareWarning" className="capitalize">
                {requiredPlan}
              </Badge>
            </div>
          </div>
          ...
          <Button
            onClick={() => router.push("/provider/settings?tab=subscription")}
            className="w-full"
          >
