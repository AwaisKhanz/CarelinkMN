...
interface ProviderVerificationGuardProps {
  children: ReactNode;
  showLoading?: boolean;
  showError?: boolean;
  redirectOnFail?: boolean;
  redirectPath?: string;
  allowPending?: boolean; // Allow access if verification is pending (default: false)
}

/**
 * Provider-specific verification guard
 * Ensures provider is verified before allowing access to features
 * Can handle pending verification state gracefully
 */
export function ProviderVerificationGuard({
  children,
  showLoading = true,
  showError = true,
  ...
  redirectPath,
  allowPending = false,
}: ProviderVerificationGuardProps) {
  const router = useRouter();
  const { provider, isLoading: providerLoading, error: providerError } = useProvider();
  const { isVerified, hasProviderProfile, isLoading: statusLoading, error: statusError } = useProviderStatus();
  ...
  useEffect(() => {
    ...
    setIsChecking(false);
  }, [hasProviderProfile, isVerified, allowPending, isLoading, redirectOnFail, redirectPath, router]);
  ...
  const isPending = hasProviderProfile && !isVerified;
